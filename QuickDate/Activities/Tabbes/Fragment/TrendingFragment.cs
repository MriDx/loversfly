    ...
    public class TrendingFragment : Android.Support.V4.App.Fragment, SpringView.IOnFreshListener
    {
        ...
        private void MainScrollEventHotOrNotOnLoadMoreEvent(object sender, EventArgs e)
        {
            try
            {
                ...
                if (item != null && HotOrNotUserAdapter.UsersDateList.Count > 10 && !MainScrollEventHotOrNot.IsLoading)
                    PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => LoadHotOrNotAsync(item.Id.ToString()) });
            }
            catch (Exception exception)
            {
                Console.WriteLine(exception);
